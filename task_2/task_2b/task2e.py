...
#num_boxes = 3
box_radius = radius
box_range = 1*box_radius # range at which a box can be picked up 
...
class boxes():
	...
	def box_iterate(self,robots): 
		...
		for i in range(robots.num_agents):
			prob_of_drop = np.random.randint(0,100)
			b = robots.holding_box[i]
			if prob_of_drop <= 3 and b != self.seq and robots.check_r[i] == True:
				robots.check_r[i] = False
				robots.holding_box[i] = -1
				self.check_b[b] = False
				self.robot_carrier[b] = -1
