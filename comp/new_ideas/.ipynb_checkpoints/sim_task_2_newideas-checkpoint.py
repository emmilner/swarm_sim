...
class swarm():
	def __init__(self,num_agents,disp):
		...
		self.dispersion_parameter = disp
	...
	def iterate(self,boxes):  # moves the robot and box positions forward in one time step
		...
		#print(np.sort(boxes.robot_carrier))
...
def random_walk(swarm,boxes):
	...
	F_agent = swarm.dispersion_parameter*R_rob*r*np.exp(-agent_distance/r)[:,np.newaxis,:]*proximity_vectors/(swarm.num_agents-1)	
	F_agent = np.sum(F_agent, axis =0).T # sum the repulsion vectors
	F_agent[boxes.robot_carrier[boxes.seq]] = 0 
...
####################################################################################
class data:
	def __init__(self,num_agents,num_boxes,anim,limit,disp):
		self.num_agents = num_agents
		self.num_boxes = num_boxes
		self.robots = swarm(self.num_agents,disp)
