...
class swarm():
	def __init__(self,num_agents,bias_R,bias_L,disp):
		self.speed = np.ones(num_agents)#speed # Agent speed 
		...
		self.mv_r_bias = np.zeros(self.num_agents)
		self.bias_L = bias_L
		self.bias_R = bias_R
	...
	def iterate(self,boxes): # moves the robot and box positions forward in one time step
		...
		self.speed = self.check_r + anti_check_r*2
...
def random_walk(swarm,boxes):
	...
	heading_x = heading_x + anti_check_r*swarm.bias_R # Biased heading behaviour if carrying a box
	
	#Bias on heading if in the exit area without a box
	robots_in_exit = np.argwhere(swarm.rob_c.T[0,:]>width-exit_width-radius)
	#if robot is in the exit then bias to move right 
	swarm.mv_r_bias[robots_in_exit] = 1 
	rob_at_r_wall = np.argwhere(swarm.rob_c.T[0,:]<radius*4)
	# if robot has reached the far right side away from exit then stop bias to move right and return to random walk 
	swarm.mv_r_bias[rob_at_r_wall] = 0 
	rob_has_box = np.argwhere(swarm.check_r==0)
	# if robot finds a box on their way to the right then stop moving right
	swarm.mv_r_bias[rob_has_box] = 0 
	where_mv_r = np.argwhere(swarm.mv_r_bias == 1)
	heading_x[where_mv_r] = heading_x[where_mv_r] - swarm.bias_L 
#	heading_y[robots_in_exit] = heading_y[robots_in_exit] - 1 
...
#	#Dispersion behaviour (from task2b_disp)
	F_agent[where_mv_r] = F_agent[where_mv_r]*swarm.dispersion_parameter
...
####################################################################################
class data:
	def __init__(self,num_agents,num_boxes,anim,limit,bias_R,bias_L,disp):
		self.num_agents = num_agents
		self.num_boxes = num_boxes
		self.robots = swarm(self.num_agents,bias_R,bias_L,disp)
