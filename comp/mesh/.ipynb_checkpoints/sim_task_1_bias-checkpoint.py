...
class swarm():
	...
	def iterate(self,boxes): # moves the robot and box positions forward in one time step
		...
		self.total = (self.rob_c.T[0]>width-exit_width-50).sum() 
		if self.total > self.num_agents/2: # if half the robots are in the delivery area 
			self.r = np.argwhere(self.state == 0) # switch states 0 to 5 (move west if without box)
			self.state[self.r] = 5 # if you don't have a box move west
		if self.total < self.num_agents/2: # now there is an acceptable number of robots in the delivery area 
			self.r = np.argwhere(self.state == 5) # switch states 5 back to 0 (random walk)
			self.state[self.r] = 0 # reset to random walk 
		#
...
def random_walk(swarm,boxes):
	...
	for K in j:
		for G in g:
			if agent_distance[K][G] > 3*radius:
				F_agent[K][0][G] = F_agent[K][0][G]*-10000 
				F_agent[K][1][G] = F_agent[K][1][G]*-10000
	...
	#for N in range(swarm.num_agents):
		#for n in m: 
	#		if N != n:
	#			F_agent[m][0][N] = F_agent[m][0][N]*10 # disperse from robots with boxes
	#			F_agent[m][1][N] = F_agent[m][1][N]*10
	#		if N == n: 
