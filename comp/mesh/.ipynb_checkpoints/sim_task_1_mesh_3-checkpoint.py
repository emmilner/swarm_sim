...
class swarm():
	def __init__(self,num_agents):
		...
		self.st4 = np.argwhere(self.state==4).flatten()
	...
	def iterate(self,boxes): # moves the robot and box positions forward in one time step
		...
		self.box_and_0 = np.intersect1d(self.st_0,self.have_a_box) # robots with state 0 that are carrying a box
		self.random_states = np.ones(np.size(self.box_and_0))*2
	...
	# the following is for plotting purposes only 
		...
		self.state_4 = self.rob_c[self.st4]
...
def random_walk(swarm,boxes):
	...
	swarm.heading_x[swarm.st2] = swarm.heading_x[swarm.st2] + 1
		...
		#S4, move west, out of delivery area 
	swarm.heading_x[swarm.st4] = swarm.heading_x[swarm.st4] - 1
