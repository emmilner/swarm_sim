...
class swarm():
	...
	def iterate(self,boxes): # moves the robot and box positions forward in one time step
		...
		self.total = (self.rob_c.T[0]>width-exit_width-50).sum() 
		if self.total > self.limit_in_delivery: # if half the robots are in the delivery area 
		...
		if self.total < self.limit_in_delivery: # now there is an acceptable number of robots in the delivery area 
