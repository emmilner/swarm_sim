...
def random_walk(swarm,boxes):
	...
	F_box = R_box*r_box*np.exp(-box_dist/r_box)[:,np.newaxis,:]*proximity_to_boxes/(swarm.num_agents-1)	
	...
	F_agent = R_rob*r_agent*np.exp(-agent_distance/r_agent)[:,np.newaxis,:]*proximity_vectors/(swarm.num_agents-1)		
