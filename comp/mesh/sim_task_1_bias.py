...
class swarm():
	...
	#	if skip == False:
		...
		total = (self.rob_c.T[0]>width-exit_width-50).sum() # total number of robots in the range of beacons
		if total > 15:
			n = np.argwhere(self.state == 0)
			self.state[n] = 5 # if you don't have a box move west
		if total < 15: # now there is an acceptable number of robots in the delivery area 
			n = np.argwhere(self.state == 5)
			self.state[n] = 0 # reset to random walk 
...
def random_walk(swarm,boxes):
	...
	swarm.heading_x[state_42] = swarm.heading_x[state_42] - 1 # middle, move west 
	swarm.heading_y[state_42] = 0#swarm.heading_y[state_4] - 1 # middle, no y movement
	swarm.heading_x[state_41] = swarm.heading_x[state_41] #- 1 #lower, no x movement 
	swarm.heading_y[state_41] = swarm.heading_y[state_41] + 1 #lower, move north 
	swarm.heading_x[state_43] = swarm.heading_x[state_43] #- 1 #upper, no x movement
	swarm.heading_y[state_43] = swarm.heading_y[state_43] - 1 #upper, move south 
	
	swarm.heading_x[state_5] = swarm.heading_x[state_5] - 1 # too many in delivery, move west 
