...
class boxes():
	def __init__(self,number_of_boxes,robots):
		...
		self.drop_off_prob = (1 - 1/(np.arange(1,self.num_boxes+1)))#drop_prob # probability out of 100 of the box being dropped by the robot (if it is not the correct box)
...
#				boxes.robot_carrier[b] = mins[b] # set the robot_carrier for box b to that robot IDz
		...
		#prob = box_drop < boxes.drop_off_prob # = 1 (drop the box) if the random number generated is below the drop off probability value
		box_drop = np.zeros(boxes.num_boxes)
		for i in range(boxes.seq,boxes.num_boxes):
			box_drop[i] = 1 - 1/(i+1)
			#(1-(1/np.arange(boxes.seq+1,boxes.num_boxes+1)))
